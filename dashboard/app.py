...
def backtest(args):
 leader=args.get("leader") or None; min_build=max(0,float(args.get("min_build",500))); max_build=float(args.get("max_build",0) or 0); min_fills=max(1,int(args.get("min_fills",2))); max_fills=max(min_fills,int(args.get("max_fills",9))); max_slippage=float(args.get("max_slippage",.01)); stake=max(.01,float(args.get("stake",50)))
