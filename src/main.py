...
def main():
    ...
    if mode!="PAPER": raise RuntimeError("v0.3 refuses to run outside PAPER mode")
    poll=float(setting("POLL_SECONDS","2")); base=float(setting("BASE_TRADE_USD","50")); window=float(setting("BUILD_WINDOW_SECONDS","30"))
    client=PolymarketPublicClient(); store=Store("data/paper.db"); watcher=WalletWatcher(client,store,leaders,base,window)
    print("Polymarket Copy Engine v0.3 | PAPER | leaders={} | build_window={}s | strategies=24".format(len(leaders),window),flush=True)
    marked=watcher.bootstrap(); print("Bootstrap complete: {} existing activity rows ignored.".format(marked),flush=True)
    stats=store.summary(); print("DB stats: fills={} builds={} api_latency avg={:.1f}s min={:.1f}s max={:.1f}s".format(stats["fills"],stats["builds"],stats["avg_latency"],stats["min_latency"],stats["max_latency"]),flush=True)
    while True:
        try:
            n=watcher.poll_once()
