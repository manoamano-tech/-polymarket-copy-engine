...
class SettlementScanner:
    ...
    def maybe_run(self,now=None):
        ...
        self.last_run=now; settled=0; counts={"ACTIVE":0,"NOT_FOUND":0,"AMBIGUOUS":0,"TOKEN_MISMATCH":0,"ERROR":0}
        for condition_id,token,event_slug in self.store.unsettled_markets():
        ...
        print("[SETTLEMENT] verified={} active={} not_found={} ambiguous={} token_mismatch={} errors={}".format(settled,counts.get("ACTIVE",0),counts.get("NOT_FOUND",0),counts.get("AMBIGUOUS",0),counts.get("TOKEN_MISMATCH",0),counts.get("ERROR",0)),flush=True)
        return settled
