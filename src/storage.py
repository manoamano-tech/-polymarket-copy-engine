...
class Store:
    ...
    def unsettled_tokens(self): return [r[0] for r in self.db.execute("SELECT DISTINCT pt.token_id FROM paper_trades pt LEFT JOIN settlements s ON s.trade_id=pt.id WHERE s.trade_id IS NULL")]
    ...
    def settle_trade(self,trade_id,ts,price,value,pnl,pct):
        self.db.execute("INSERT OR IGNORE INTO settlements(trade_id,settled_at,settlement_price,value_usd,realized_pnl_usd,realized_pnl_pct) VALUES(?,?,?,?,?,?)",(trade_id,ts,price,value,pnl,pct)); self.db.commit()
    def summary(self):
