...
CREATE TABLE IF NOT EXISTS private_raw_fills (id INTEGER PRIMARY KEY AUTOINCREMENT, profile_id INTEGER NOT NULL, fingerprint TEXT NOT NULL, observed_at REAL NOT NULL, trade_ts INTEGER NOT NULL, wallet TEXT NOT NULL, transaction_hash TEXT, market TEXT NOT NULL, event TEXT, outcome TEXT, side TEXT NOT NULL, token_id TEXT NOT NULL, leader_price REAL NOT NULL, leader_usdc REAL NOT NULL, sport TEXT, league TEXT, UNIQUE(profile_id,fingerprint));
CREATE TABLE IF NOT EXISTS polymarket_accounts (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL UNIQUE, wallet_address TEXT NOT NULL, wallet_type TEXT NOT NULL DEFAULT 'DEPOSIT_WALLET', connection_status TEXT NOT NULL DEFAULT 'READ_ONLY', connected_at REAL NOT NULL, updated_at REAL NOT NULL, FOREIGN KEY(user_id) REFERENCES users(id));
...
class Store:
    ...
    def _migrate(self):
        self._ensure_column("raw_fills","sport","TEXT"); self._ensure_column("raw_fills","league","TEXT"); self._ensure_column("copy_profiles","user_id","INTEGER"); self._ensure_column("copy_profiles","source_wallet","TEXT"); self._ensure_column("copy_profiles","source_type","TEXT DEFAULT 'PUBLIC'")
