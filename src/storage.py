...
class Store:
    ...
    def summary(self):
        fills=self.db.execute("SELECT COUNT(*),COALESCE(AVG(latency_seconds),0),COALESCE(MIN(latency_seconds),0),COALESCE(MAX(latency_seconds),0) FROM raw_fills").fetchone(); builds=self.db.execute("SELECT COUNT(*) FROM paper_builds").fetchone()[0]; sessions=self.db.execute("SELECT COUNT(*) FROM position_sessions").fetchone()[0]
        return {"fills":fills[0],"avg_latency":fills[1],"min_latency":fills[2],"max_latency":fills[3],"builds":builds,"sessions":sessions}
